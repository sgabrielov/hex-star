...
# File: new_heuristic_opt.py
#
# Code summary:
#
# H* + Learned Heuristic + (optional) Optuna Hyperparameter Tuning
# ------------------------------------------------------------------
# - Learns a time-to-goal heuristic via supervised regression
# - The time-to-goal here is computed using time_to_goal() from node directly to goal which ignores boundaries
# - The training is relatively fast
# - Can train either:
...
def random_rollout_training_samples(problem, num_rollouts=400, max_steps=50):
    # ###############################################################
    # Use random rollouts in the map to collect (features, true_time_to_goal) pairs.

    # Note: This learns a heuristic for THIS map and THIS goal (problem.goal_loc).
    # ###############################################################
    X_list = []
    y_list = []
    ...
    for rollout in range(num_rollouts):
        ...
        for _ in range(max_steps):
            ...
            # Compute true time-to-goal using the analytic time_to_goal heuristic
            node_obj = problem.Node(next_state, None, None, 0, problem)
...
class HybridHeuristic:
    ...
    # If you want strict admissibility, you could instead use:
    ...
    def __call__(self, node):
        ...
        # safer variant: return min(h_learn, h_ana)
        # return self.alpha * h_learn + (1.0 - self.alpha) * h_ana
...
def evaluate_multiple_starts(hex_map, obstacle_map, goal_loc, hex_radius,
    ...
    for _ in range(num_starts):
        ...
        # Get path solution from start to goal based on three different methods
        # Original H*, Learned Heuristic, and a Hybrid combination between Original H* and Learned Heuristic
...
def run_pipeline(pkl_path, model_path="learned_heuristic.pt", use_optuna=True):
    # --------------------------
    # Load map
    # --------------------------
    ...
    X, y = random_rollout_training_samples(train_problem, num_rollouts=1500, max_steps=100)
    ...
    # --------------------------
    # (Demonstrate load) - reload from disk to ensure compatibility
    # --------------------------
    # For Optuna, we do not know hidden_sizes from the outside; however, in this
    # demo we just re-use the already loaded `model` directly, so re-loading is optional.
    # If you want to re-load with correct hidden_sizes, you would need to pass them.
    # Here, to keep it simple, we skip re-loading and just use `model`.
...
if __name__ == "__main__":
    ...
    if use_optuna:
        ...
        model_path_fname = "learned_heuristic.pt"
