...
def snapshot_paths_hexcoord_set(snapshot: dict, solid: bool = True) -> set:
    """
    Extract hex coordinates from a processed progress snapshot.
    ...
    ----------
    snapshot : dict
        Processed snapshot (i.e. after snapshot_with_paths_fn has been applied).
    solid : bool
        If True, include full solid path hexes.
        If False, include node locations only.
    ...
    # Support both single-search and bidirectional snapshot formats
    if "forward_top" in snapshot or "reverse_top" in snapshot:
        groups = [
            snapshot.get("forward_top", []),
            snapshot.get("reverse_top", []),
        ]
    else:
        groups = [
            snapshot.get("top", []),
    ...
    for entries in groups:
        for entry in entries:
            ...
            if solid:
                full_path = construct_full_solution(path)
                if full_path is not None:
                    out.update(full_path)
            else:
                out.update(node.location for node in path if node is not None)
